// packages/shared/src/types/story.ts
// Phase 1 base fields + Phase 2-A UI extension fields

// ---- Story (full row, password_hash excluded from VIEW) ----

export interface Story {
  // Phase 1 base fields
  id: string;
  title: string;
  world_setting: string;
  story: string;
  character_name: string;
  character_setting: string;
  characters: string;
  user_note: string;
  system_rules: string;
  use_latex: boolean;
  is_public: boolean;
  has_password: boolean; // stories_safe VIEW: (password_hash IS NOT NULL)
  owner_uid: string | null;
  created_at: string;
  updated_at: string;

  // Phase 2-A UI extension fields
  description: string;
  tags: string[];
  icon: string;
  banner_gradient: string;
  play_count: number;
  like_count: number;
  badge: 'new' | 'hot' | null;
  is_featured: boolean;
  owner_name: string;
}

// ---- StoryListItem — 카드/행 렌더링용 경량 타입 ----

export interface StoryListItem {
  id: string;
  title: string;
  description: string;
  tags: string[];
  icon: string;
  banner_gradient: string;
  play_count: number;
  like_count: number;
  badge: 'new' | 'hot' | null;
  is_featured: boolean;
  has_password: boolean;
  owner_name: string;
  created_at: string;
}

// ---- StoryStats — Hero 섹션 통계 ----

export interface StoryStats {
  total_stories: number;
  total_plays: number;
  total_authors: number;
}

// ---- Create / Update DTOs ----

export interface StoryCreateInput {
  title: string;
  world_setting?: string;
  story?: string;
  character_name?: string;
  character_setting?: string;
  characters?: string;
  user_note?: string;
  system_rules?: string;
  use_latex?: boolean;
  is_public?: boolean;
  password_hash?: string;
  // Phase 2-A
  description?: string;
  tags?: string[];
  icon?: string;
  banner_gradient?: string;
  owner_name?: string;
}

export type StoryUpdateInput = Partial<StoryCreateInput>;

// ---- StoryPreset (게임 시작 옵션) ----

export interface StoryPreset {
  characterName: string;
  characterSetting: string;
  useLatex: boolean;
  narrativeLength: number;
}

// ---- Preset (관리자 프리셋) ----

export interface Preset {
  // Phase 1 base fields
  id: string;
  title: string;
  is_default: boolean;
  world_setting: string;
  story: string;
  characters: string;
  character_name: string;
  character_setting: string;
  user_note: string;
  system_rules: string;
  use_latex: boolean;
  created_at: string;
  updated_at: string;

  // Phase 2-A extension fields
  genre: string;
  icon: string;
  status_preset_id: string | null;
}

export type PresetCreateInput = Omit<Preset, 'id' | 'created_at' | 'updated_at'>;
export type PresetUpdateInput = Partial<PresetCreateInput>;
