#!/bin/bash
# Update enhanced prompts via Supabase REST API
...
# Get current config
CURRENT_CONFIG=$(curl -s "$SUPABASE_URL/rest/v1/config?id=eq.prompt_config&select=value" \
...
# Update with enhanced prompts (using SQL via RPC)
# We'll use the Supabase SQL endpoint or direct UPDATE

# For now, let's provide the SQL that can be run manually:
echo ""
echo "=== Manual SQL Update Required ==="
echo ""
echo "Run the following SQL in your Supabase SQL Editor or via psql:"
echo ""
cat << 'EOF'
...
SET value = jsonb_set(
  value,
  '{system_preamble}',
  '당신은 능숙한 인터랙티브 소설 작가이자 스토리텔러입니다.
...
- **미스터리와 호기심**을 자극하는 요소를 배치하세요'::jsonb
),
updated_at = now()
WHERE id = 'prompt_config';

-- Update memory_system_instruction
UPDATE story_game.config
SET value = jsonb_set(
  value,
  '{memory_system_instruction}',
  '당신은 이야기의 연속성을 관리하는 기억 관리자입니다. 대화 내용을 분석하여 중요한 요소를 체계적으로 정리하세요.
...
- **최대 {max_chars}자**를 넘지 마세요 (핵심만 간결하게)'::jsonb
),
updated_at = now()
WHERE id = 'prompt_config';

-- Update memory_request
UPDATE story_game.config
SET value = jsonb_set(
  value,
  '{memory_request}',
...
echo ""
echo "Or run migration file: supabase/migrations/20260331020000_enhanced_prompt_config.sql"
