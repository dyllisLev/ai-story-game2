// backend/src/routes/admin/verify.ts
// GET /api/admin/verify — verify if current user has admin role (server-side check)
...
export default async function adminVerifyRoute(app: FastifyInstance) {
  app.get('/admin/verify', async (request, reply) => {
    ...
    const response: VerifyAdminResponse = {
      isAdmin,
      userId: user.id,
      email: user.email,
      role: actualRole,
