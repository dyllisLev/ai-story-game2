...
export class MetricsRegistry {
  ...
  private static readonly MAX_HISTOGRAM_SIZE = 10000; // Prevent unbounded growth
  ...
  getMetrics(): Metric[] {
    ...
    for (const [key, histogram] of this.histograms) {
      ...
      metrics.push(
        { name: `${name}_count`, type: 'counter', value: values.length, labels: histogram.labels, timestamp: now },
        { name: `${name}_sum`, type: 'counter', value: values.reduce((a, b) => a + b, 0), labels: histogram.labels, timestamp: now },
        { name: `${name}_avg`, type: 'gauge', value: this.average(values), labels: histogram.labels, timestamp: now },
        { name: `${name}_p50`, type: 'gauge', value: this.percentile(values, 50), labels: histogram.labels, timestamp: now },
        { name: `${name}_p95`, type: 'gauge', value: this.percentile(values, 95), labels: histogram.labels, timestamp: now },
        { name: `${name}_p99`, type: 'gauge', value: this.percentile(values, 99), labels: histogram.labels, timestamp: now }
      );
    }
 ...
 */
export function trackDatabaseQuery(table: string, operation: string, duration: number, error?: boolean): void {
  const registry = getMetricsRegistry();
 ...
 */
export function trackCacheOperation(operation: 'hit' | 'miss' | 'set' | 'delete', key?: string): void {
  const registry = getMetricsRegistry();
  const labels: Record<string, string> = key ? { operation, key } : { operation };
 ...
 */
export function trackApplicationError(errorType: string, message: string): void {
  const registry = getMetricsRegistry();
