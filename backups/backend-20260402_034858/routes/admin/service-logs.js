...
export default async function adminServiceLogsRoute(app) {
    ...
    app.get('/admin/service-logs/stats', async (request, reply) => {
        ...
        const [{ count: totalCount }, { count: errorCount }, { count: total_requests_today }, { data: avgData }, { data: hourlyData },] = await Promise.all([
        ...
        const error_rate_24h = totalCount && totalCount > 0
            ? Math.round(((errorCount ?? 0) / totalCount) * 10000) / 100
            : 0;
        const avg_response_ms = Math.round(avgData ?? 0);
        // Build 24-slot array from the RPC result (array of {hour, count})
        const hourlyCounts = {};
        for (let h = 0; h < 24; h++)
            hourlyCounts[h] = 0;
        (hourlyData ?? []).forEach(({ hour, count }) => { hourlyCounts[hour] = count; });
        const response = {
            error_rate_24h,
            avg_response_ms,
