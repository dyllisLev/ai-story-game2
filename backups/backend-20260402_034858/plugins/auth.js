 ...
 */
export function requireAdminBasicAuth(request) {
 ...
 */
export function requireAdminWithBasicAuth(request) {
 ...
 */
export async function verifyResourceOwner(app, request, table, id) {
    ...
    if (data.owner_uid !== user.id && user.role !== 'admin') {
        throw { statusCode: 403, code: 'FORBIDDEN', message: '이 리소스를 수정할 권한이 없습니다' };
    }
}
// 세션 토큰 검증 (익명 사용자용)
export async function verifySessionAccess(app, request, sessionId) {
